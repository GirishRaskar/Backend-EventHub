...
router.post('/postData', async (req, res) => {
  try {
      const { name, email, mobile, batch } = req.body;
      ...
      // Create a new student document (MongoDB will auto-generate `_id`)
      const newStudent = new Student({ name, email, mobile, batch });
      const result = await newStudent.save();

      console.log('Student admission added:', result);
      res.status(201).json({
          message: 'Student admitted successfully!',
          insertedId: result._id, // Auto-generated by MongoDB
          insertedData: result
      });
